...
export default function AdminDashboard() {
  ...
  // Add story function for semantic configuration
  const addStory = () => {
            ...
            <Tabs defaultValue="basic" className="space-y-6">
              ...
              <TabsContent value="stories" className="space-y-4">
                ...
                {mentorStories.length > 0 && (
                  <div className="space-y-3">
                    <h5 className="font-medium">Existing Stories</h5>
                    {mentorStories.map((story, index) => (
                      <Card key={story.id || index} className="p-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="text-xs bg-slate-100 px-2 py-1 rounded">{story.category}</span>
                            <h6 className="font-medium mt-1">{story.title}</h6>
                            <p className="text-sm text-slate-600 mt-1 line-clamp-2">{story.story}</p>
                          </div>
